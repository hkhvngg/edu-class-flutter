...
class LoginCubit extends Cubit<LoginState> {
  ...
  Future<void> login(String email, String password) async {
    ...
    try {
      ...
      if (email == 'admin@educlass.com' && password == 'admin123') {
        try {
          userCredential = await _authService.signIn(email, password).timeout(const Duration(seconds: 15));
        } catch (e) {
          print("DEBUG: Admin account not found or invalid, creating new one...");
          userCredential = await _authService.signUp(email, password).timeout(const Duration(seconds: 15));
      ...
      UserModel? userModel = await _databaseService.getUser(userCredential.user!.uid)
      ...
      if (userModel != null) {
        ...
        emit(const LoginFailure(message: 'Tài khoản Auth tồn tại nhưng không có dữ liệu trong Firestore!'));
      }
    } on TimeoutException {
      emit(const LoginFailure(message: 'Quá thời gian kết nối! Vui lòng kiểm tra mạng hoặc cấu hình Firebase.'));
